...
https://supabase.com/dashboard/project/_/settings/api`);
          let { storage: s11, getAll: n11, setAll: i2, setItems: a2, removedItems: o2 } = function(e12, t12) {
            let r12, s12, n12 = e12.cookies ?? null, i3 = e12.cookieEncoding, a3 = {}, o3 = {};
            ...
            return t12 ? { getAll: r12, setAll: s12, setItems: a3, removedItems: o3, storage: { isServer: true, getItem: async (e13) => {
              ...
              t13.endsWith("-code-verifier") && await rT({ getAll: r12, setAll: s12, setItems: { [t13]: n13 }, removedItems: {} }, { cookieOptions: e12?.cookieOptions ?? null, cookieEncoding: i3 }), a3[t13] = n13, delete o3[t13];
              ...
              let n13 = await r12([t13]), i4 = (n13?.map(({ name: e13 }) => e13) || []).filter((e13) => r_(e13, t13)), a4 = { ...rv, ...e12?.cookieOptions, maxAge: 0 };
          ...
          return l2.auth.onAuthStateChange(async (e12) => {
            (Object.keys(a2).length > 0 || Object.keys(o2).length > 0) && ("SIGNED_IN" === e12 || "TOKEN_REFRESHED" === e12 || "USER_UPDATED" === e12 || "PASSWORD_RECOVERY" === e12 || "SIGNED_OUT" === e12 || "MFA_CHALLENGE_VERIFIED" === e12) && await rT({ getAll: n11, setAll: i2, setItems: a2, removedItems: o2 }, { cookieOptions: r11?.cookieOptions ?? null, cookieEncoding: r11?.cookieEncoding ?? "base64url" });
          }), l2;
        }("https://gzebdkjlwtcgdjxbxpwf.supabase.co", "sb_publishable_kgDchPHKi_RbCbbDyax9xg_LIAY8Asl", { cookies: { getAll: () => e10.cookies.getAll(), setAll(r11) {
        ...
        if ((/* @__PURE__ */ new Map([["X-Frame-Options", "DENY"], ["X-Content-Type-Options", "nosniff"], ["Referrer-Policy", "strict-origin-when-cross-origin"], ["Permissions-Policy", "camera=(), microphone=(), geolocation=()"]])).forEach((e11, r11) => {
          t10.headers.set(r11, e11);
        }), t10.headers.set("Strict-Transport-Security", "max-age=63072000; includeSubDomains; preload"), t10.headers.set("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: blob: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https://*.supabase.co https://*.supabase.in; media-src 'self' blob:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'"), "/admin-login" === n10) {
          if (s10) {
            let { data: t11 } = await r10.from("admin_users").select("id").eq("user_id", s10.id).maybeSingle();
            if (t11) {
...
var BuildId = "2YqB6Oykybihahb5iZdvR";
